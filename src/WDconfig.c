...
static void SetDefaultConfiguration(WaveDumpConfig_t *WDcfg) {
	...
	WDcfg->FPIOtype = 0;
	WDcfg->Number_of_events_CUT = 1000000;//default no cut value
...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
        ...
        // Number of events to stop the run, if present
		if (strstr(str, "NUMBER_OF_EVENTS")!=NULL) {
			read = fscanf(f_ini, "%d", &WDcfg->Number_of_events_CUT);
			printf("%d: NUMBER_OF_EVENTS\n", WDcfg->Number_of_events_CUT);
			continue;
		}
